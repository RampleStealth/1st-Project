...
type SyncStateRow = { mailbox_account_id: string; applied_history_id: string | null; pending_history_id: string | null; initial_sync_status: "pending" | "running" | "complete" | "failed"; reconciliation_due_at: Date | null; last_successful_sync_at: Date | null; };

function toState(row: SyncStateRow): MailboxSyncState {
  return { mailboxAccountId: row.mailbox_account_id, appliedHistoryId: row.applied_history_id, pendingHistoryId: row.pending_history_id, initialSyncStatus: row.initial_sync_status, reconciliationDueAt: row.reconciliation_due_at, lastSuccessfulSyncAt: row.last_successful_sync_at };
}
...
export async function claimDueReconciliations(limit: number, reconciliationMinutes: number): Promise<string[]> {
  return withTransaction(async (client) => {
    const result = await client.query<{ mailbox_account_id: string }>(`WITH due AS (SELECT mailbox_account_id FROM mailbox_sync_state WHERE reconciliation_due_at <= now() ORDER BY reconciliation_due_at ASC FOR UPDATE SKIP LOCKED LIMIT $1) UPDATE mailbox_sync_state state SET reconciliation_due_at=now() + ($2 * interval '1 minute'),updated_at=now() FROM due WHERE state.mailbox_account_id=due.mailbox_account_id RETURNING state.mailbox_account_id`, [limit, reconciliationMinutes]);
