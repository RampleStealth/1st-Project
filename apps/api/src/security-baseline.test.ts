...
function dependencies(options: { config?: AppConfig; rateLimiter?: RateLimiter; calls?: { redisSet: number } } = {}): ApiAppDependencies {
  ...
  return {
    ...
    withTransaction: async (fn) => fn(client), findMailboxForUser: async () => null, searchMailboxThreads: async () => ({ threads: [], nextPageToken: null }), ensureMailboxSyncState: async () => undefined, recordPendingHistory: async () => undefined, enqueueSync: async () => undefined,
