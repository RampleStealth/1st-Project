...
async function makeApp(options: { permission?: string; draft?: any; eligibility?: { draftId: string; writeGranted: boolean } | null; recoveryCommand?: { id: string; status: string } | null; config?: AppConfig } = {}) {
  ...
  const dependencies: ApiAppDependencies = {
    ...
    findMailboxForUser: async (id, user) => id === mailboxId && user === ownerId ? mailbox() : null, searchMailboxThreads: async () => ({ threads: [], nextPageToken: null }), ensureMailboxSyncState: async () => undefined, recordPendingHistory: async () => undefined, enqueueSync: async () => undefined,
