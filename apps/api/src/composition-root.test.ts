...
const config: AppConfig = {
  ...
  TRUST_PROXY: false,
  API_BODY_LIMIT_BYTES: 600 * 1024,
  WEBHOOK_BODY_LIMIT_BYTES: 64 * 1024,
  PORT: 4000,
  WEB_PORT: 3000
};
...
function makeApiDependencies(events: string[] = []): ApiAppDependencies {
  ...
  return {
    ...
    isDraftStateConflictError: () => false,
...
function makeFactories(events: string[], dependencies: ApiAppDependencies): ProductionDependencyFactories {
  return {
    ...
    searchMailboxThreads: async (_config, mailbox, terms, pageToken, limit) => dependencies.searchMailboxThreads(mailbox, terms, pageToken, limit),
...
test("application factory imports without startup side effects and registers the API surface", async () => {
  ...
  for (const [method, url] of [
    ...
    ["GET", "/v1/mailboxes/:mailboxId/search"],
