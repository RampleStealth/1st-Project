...
type Message = { id: string; from: string | null; to: string[]; labels?: string[]; subject: string | null; sentAt: string | null; attachments: Array<{ filename: string; mimeType: string; size: number | null }>; plainText: string; sanitizedHtml: string | null; renderingState: "ready" | "fallback" | "failed" };
...
export function ThreadReader({ mailboxId, threadId, view, onArchived, onUnread, onClose, onEditDraft, onPermissionRequired }: { mailboxId: string; threadId?: string; view?: string; onArchived?: () => void; onUnread?: () => void; onClose?: () => void; onEditDraft?: (draftId: string) => void; onPermissionRequired?: () => void }) {
  ...
  const searchLabels = useMemo(() => new Set(thread?.messages.flatMap((message) => message.labels ?? []) ?? []), [thread]);
