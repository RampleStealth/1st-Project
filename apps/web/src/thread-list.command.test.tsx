...
describe("confirmed thread command UI",()=>{
  it.each(["inbox","all","sent","drafts"])("archive confirmation is scoped in %s",async view=>{renderList(view);await screen.findByLabelText("A from A");await act(async()=>window.dispatchEvent(new CustomEvent("aio:thread-command-confirmed",{detail:{threadId:"thread-a",action:"archive"}})));if(view==="inbox")await waitFor(()=>expect(screen.queryByLabelText("A from A")).toBeNull());else expect(screen.getByLabelText("A from A")).toBeTruthy();});
  it("updates only the confirmed unread row",async()=>{renderList("inbox");await screen.findByLabelText("A from A");await act(async()=>window.dispatchEvent(new CustomEvent("aio:thread-command-confirmed",{detail:{threadId:"thread-a",action:"mark-unread"}})));await waitFor(()=>expect(screen.getByLabelText("1 unread messages")).toBeTruthy());expect(screen.getByLabelText("B from B")).toBeTruthy();});
